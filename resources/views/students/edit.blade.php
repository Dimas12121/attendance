...
<div class="glass-card" style="max-width: 700px; margin: 0 auto;">
    <h2 style="margin-bottom: 2rem;">✏️ Edit Data Siswa: {{ $student->name }}</h2>
    ...
    <form action="{{ route('students.update', $student) }}" method="POST">
        ...
        <div style="margin-bottom: 2.5rem; text-align: center;">
            ...
            <div id="status" style="margin-top: 1rem; color: var(--text-dim); font-size: 0.9rem;">Biarkan kosong jika tidak ingin merubah data wajah.</div>
            <button type="button" id="captureBtn" class="btn" style="margin-top: 1rem; width: 100%; background: rgba(255,255,255,0.05); border: 1px solid var(--border);" disabled>Ambil Wajah Baru</button>
        </div>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem; margin-bottom: 1.5rem;">
            ...
            <div>
                ...
                <input type="text" name="student_id" required value="{{ $student->student_id }}">
            </div>
        </div>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem; margin-bottom: 2rem;">
            <div>
                ...
                <input type="text" name="phone_parent" required value="{{ $student->phone_parent }}">
            </div>
            <div>
                <label style="display: block; margin-bottom: 0.5rem; font-weight: 600;">Tanggal Lahir</label>
                <input type="date" name="birth_date" required value="{{ $student->birth_date }}">
            </div>
        </div>

        <input type="hidden" name="face_descriptor" id="face_descriptor">

        <button type="submit" class="btn" style="width: 100%;">Perbarui Data Siswa</button>
        <a href="{{ route('students.index') }}" style="display: block; text-align: center; margin-top: 1rem; color: var(--text-dim); text-decoration: none;">Batal</a>
    </form>
</div>
...
<script>
    ...
    captureBtn.addEventListener('click', async () => {
        ...
        if (detection) {
            ...
            status.innerHTML = "<b style='color: #4ade80'>✅ Wajah Baru Berhasil Terdeteksi!</b>";
        } else {
            alert("Wajah gagal dideteksi.");
        }
    });
